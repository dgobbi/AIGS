...
int MicronTrackerInterface::mtInit()
{
	...
	this->m_pTempMarkerForAddingFacet = new Marker();
	return 1;
}
...
int MicronTrackerInterface::mtSetupCameras()
{
	...
	{
		this->m_errorString = mtUtils::handleErrors(result);
		return 0;
	}
}
...
void MicronTrackerInterface::mtFindIdentifiedMarkers()
{
	...
	for (markerNum = 1; markerNum <= markersCollection->count(); markerNum++)
	{
		...
		Collection* totalFacetsCollection = new Collection(this->m_pCurrTempMarker->getTemplateFacets());
			
		this->m_vIdentifiedMarkersName.push_back(this->m_pCurrTempMarker->getName());

		if (m_pCurrTempMarker->wasIdentified(this->m_pCurrCam) != 0)
		{
			Collection* facetsCollection = new Collection(this->m_pCurrTempMarker->identifiedFacets(this->m_pCurrCam));

			// Adjust the colour temperature if we see a CoolCard marker
			if ((0 == strncmp(m_pCurrTempMarker->getName(), "COOL", 4)) ||  
				(0 == strncmp(m_pCurrTempMarker->getName(), "cool", 4)) ||
				(0 == strncmp(m_pCurrTempMarker->getName(), "Cool", 4))
			   )
			{
...
//			delete totalFacetsCollection;
			...
			Xform3D* Marker2CurrCameraXf = m_pCurrTempMarker->marker2CameraXf(this->m_pCurrCam->getHandle());
...
{
	return this->m_pCameras->setShutterPreference(n);
}

/****************************/
double MicronTrackerInterface::mtGetShutterPreference()
{
	return this->m_pCameras->getShutterPreference();
}

/****************************/
int MicronTrackerInterface::mtSetTemplMatchTolerance(double matchTolerance)
{
	return this->m_pMarkers->setTemplateMatchToleranceMM(matchTolerance);
}
...
{
	return this->m_pMarkers->setPredictiveFramesInterleave(predictiveInterleave);
}
...
int MicronTrackerInterface::mtSetTemplateName(int index, char* templName)
{
	...
	char* oldN = this->m_pMarkers->getTemplateItemName(index);
	...
	result = mtUtils::renameFile(oldN, templName, "Markers");
	if (result != mtOK)
...
{	
	...
	{
		char* templateName = this->m_pMarkers->getTemplateItemName(index);
		result = mtUtils::removeFile(templateName, "Markers");
		if (result != 0)
...
/****************************/
int MicronTrackerInterface::mtGetLeftRightImageArrayHalfSize(unsigned char** &leftImageArray, unsigned char** &rightImageArray, int xResolution, int yResolution, int index)
{
	bool result = 0;
	if (index == -1)
